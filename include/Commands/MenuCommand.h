...
public:
	MenuCommand(GameStateModel *gameStateModel, SceneManager *sceneManager,
							std::function<std::unique_ptr<GameState>()> stateFactory, const std::string &commandName)
			: _gameStateModel(gameStateModel), _sceneManager(sceneManager), _stateFactory(stateFactory),
...
public:
	PopSceneCommand(SceneManager *sceneManager) : _sceneManager(sceneManager) {}
	...
	std::string getName() const override { return "Pop Scene Command"; }
