...
static float clamp01_from100(float v) { return std::clamp(v / 100.0f, 0.0f, 1.0f); }
...
float AudioManager::getVolume() const { return _volume; }
...
void AudioManager::setSoundEffectsVolume(float volume) { _soundEffectsVolume = std::clamp(volume, 0.0f, 100.0f); }

float AudioManager::getSoundEffectsVolume() const { return _soundEffectsVolume; }

void AudioManager::setSoundEffectsMuted(bool muted) { _soundEffectsMuted = muted; }

bool AudioManager::isSoundEffectsMuted() const { return _soundEffectsMuted; }
...
float AudioManager::getMusicVolume() const { return _musicVolume; }
...
bool AudioManager::isMusicMuted() const { return _musicMuted; }
