...
InGameMenuController::InGameMenuController(InGameScene* inGameScene, GameModel* gameModel)
...
void InGameMenuController::createInGameMenu() {
    _menuSystem = std::make_shared<Menu>("Game Data selection", true);
		...
		Reset->setCommand(std::make_unique<ResetCommand>(_gameModel));
		_menuSystem->addItem(Reset);

		auto Undo = std::make_shared<MenuItem>("UNDO", true);
		Undo->setCommand(std::make_unique<UndoCommand>(_gameModel));
		_menuSystem->addItem(Undo);

		auto Redo = std::make_shared<MenuItem>("REDO", true);
		Redo->setCommand(std::make_unique<RedoCommand>(_gameModel));
