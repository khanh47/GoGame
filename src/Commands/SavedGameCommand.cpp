...
public:
	...
	void execute() override {
		if (!_m)
			return;
		if (!_m->getSelectedGameData().has_value())
			return;
...
public:
	...
	void execute() override {
		...
		if (!ok)
			std::cerr << "Save to selected failed\n";
	}
	bool canUndo() const override { return false; }
	bool canRedo() const override { return false; }
	void undo() override {}
...
public:
	...
	void execute() override {
		if (_s)
			_s->closeGameDataInputPopupAndCreate();
...
public:
	SavedGameSelectCommand(DataManager *data, const std::string &file) : _data(data), _filename(file) {}
