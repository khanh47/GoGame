...
std::vector<std::vector<int>> Game::getValidPlayer1Map() {
		return _board->getValidPlayer1Map();
}

std::vector<std::vector<int>> Game::getValidPlayer2Map() {
		return _board->getValidPlayer2Map();
}
...
void Game::DetermineTerritories() {
    ...
    for (int i = 0; i < _board->GetNumRows(); i++) {
        for (int j = 0; j < _board->GetNumCols(); j++) {
            if (visited[i][j] || _board->getValue(i, j) != 0) continue;
...
void Game::CheckValid(int CurrentPlayer, int OpponentPlayer) {
    ...
    for (int i = 0; i < _board->GetNumRows(); i++) {
        for (int j = 0; j < _board->GetNumCols(); j++) {
            if (_board->getValue(i, j) != CurrentPlayer || visited[i][j]) continue;
            ...
            if (EmptySpots.size() == 1 && !_board->Alive(EmptySpots[0].first, EmptySpots[0].second)) {
                if ((CurrentPlayer == 1 && _board->getValidPlayer1(EmptySpots[0].first, EmptySpots[0].second) == 0) || (CurrentPlayer == 2 && _board->getValidPlayer2(EmptySpots[0].first, EmptySpots[0].second) == 0))
    ...
    for (int i = 0; i < _board->GetNumRows(); i++) {
        for (int j = 0; j < _board->GetNumCols(); j++) {
            if (_board->getValue(i, j) == 0 && !_board->Alive(i, j)) {
                if (CurrentPlayer == 1 && _board->getValidPlayer1(i, j)) continue;
                if (CurrentPlayer == 2 && _board->getValidPlayer2(i, j)) continue;
