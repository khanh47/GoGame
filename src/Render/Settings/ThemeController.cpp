...
ThemeController::ThemeController() { syncFromSettings(); }
...
ThemeType ThemeController::getTheme() const { return _theme; }
...
int ThemeController::getColor() const { return _colorIndex; }
...
void ThemeController::drawColorSelection(Vector2 position, Vector2 size) const {
	...
	for (size_t i = 0; i < palette.size(); i++) {
		...
		if (isSelected) {
			DrawRectangle((int)(swatchX - 2), (int)(swatchY - 2), (int)(COLOR_SWATCH_SIZE + 4), (int)(COLOR_SWATCH_SIZE + 4),
										Color{100, 149, 237, 255});
		}

		DrawRectangle((int)swatchX, (int)swatchY, (int)COLOR_SWATCH_SIZE, (int)COLOR_SWATCH_SIZE, palette[i]);
		DrawRectangleLinesEx({swatchX, swatchY, COLOR_SWATCH_SIZE, COLOR_SWATCH_SIZE}, 1.0f, DARKGRAY);
	}
}
...
const char *ThemeController::getThemeName(ThemeType theme) {
	...
	case ThemeType::Classic:
		return "Classic";
	case ThemeType::Cartoon:
		return "Cartoon";
	case ThemeType::Aesthetic:
		return "Aesthetic";
	default:
		return "Unknown";
	}
