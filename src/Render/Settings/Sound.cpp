...
SoundSettings::SoundSettings(AudioManager *audioManager) : _audioManager(audioManager) {
...
void SoundSettings::initializeMenuItems() {
	...
	// Sound Effects item
	auto soundEffectsItem = std::make_unique<ExpandableMenuItemView>(Vector2{_position.x, _position.y},
																																	 Vector2{_size.x, 40.0f}, "Sound Effects");
	soundEffectsItem->setExpandedContentSize({_size.x, 60.0f});
	soundEffectsItem->setRenderExpandedContent(
			[this](Vector2 pos, Vector2 size) { _soundEffectsController->render(pos, size); });
	_menuItems.push_back(std::move(soundEffectsItem));

	// Background Music item
	auto musicItem = std::make_unique<ExpandableMenuItemView>(Vector2{_position.x, _position.y + 45.0f},
																														Vector2{_size.x, 40.0f}, "Background Music");
	musicItem->setExpandedContentSize({_size.x, 60.0f});
	musicItem->setRenderExpandedContent([this](Vector2 pos, Vector2 size) { _musicController->render(pos, size); });
