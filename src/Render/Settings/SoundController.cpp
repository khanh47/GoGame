...
SoundController::SoundController(AudioManager *audioManager) : _audioManager(audioManager) { syncFromAudioManager(); }
...
float SoundController::getVolume() const { return _volume; }
...
bool SoundController::isMuted() const { return _muted; }
...
bool SoundController::isSliderClicked(Vector2 mousePos, Vector2 sliderPos, float sliderWidth) const {
	Rectangle sliderRect = {sliderPos.x - SLIDER_KNOB_RADIUS, sliderPos.y - SLIDER_KNOB_RADIUS,
													sliderWidth + SLIDER_KNOB_RADIUS * 2, SLIDER_HEIGHT + SLIDER_KNOB_RADIUS * 2};
