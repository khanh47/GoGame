...
std::unique_ptr<MenuCommand> createPlayCommand(GameStateModel *gameStateModel, SceneManager *sceneManager,
	...
	return std::make_unique<MenuCommand>(
			gameStateModel, sceneManager,
			std::function<std::unique_ptr<GameState>()>([gameMode]() { return std::make_unique<InGameState>(gameMode); }),
...
std::unique_ptr<ICommand> createExitCommand() { return std::make_unique<ExitCommand>(); }
