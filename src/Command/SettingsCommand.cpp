...
// SetThemeCommand
SetThemeCommand::SetThemeCommand(ThemeTarget target, ThemeType theme)
    : _target(target), _theme(theme) {}
...
std::unique_ptr<ICommand> SetThemeCommand::clone() const {
    return std::make_unique<SetThemeCommand>(_target, _theme);
}

// SetColorCommand
SetColorCommand::SetColorCommand(ColorTarget target, int colorIndex)
    : _target(target), _colorIndex(colorIndex) {}
