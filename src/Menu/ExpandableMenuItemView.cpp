...
Rectangle ExpandableMenuItemView::GetHeaderBoundingBox() const {
    return { _position.x, _position.y, _size.x, _size.y };
}

void ExpandableMenuItemView::setHovered(bool hovered) {
    _isHovered = hovered;
}

bool ExpandableMenuItemView::getHovered() const {
    return _isHovered;
}

void ExpandableMenuItemView::setExpanded(bool expanded) {
    _isExpanded = expanded;
}

bool ExpandableMenuItemView::isExpanded() const {
    return _isExpanded;
}

void ExpandableMenuItemView::toggleExpanded() {
...
void ExpandableMenuItemView::render() const {
    ...
    if (_isExpanded) {
        ...
        DrawRectangle((int)contentPos.x, (int)contentPos.y, 
                      (int)_expandedContentSize.x, (int)_expandedContentSize.y, _expandedBackgroundColor);
        DrawRectangleLinesEx({contentPos.x, contentPos.y, _expandedContentSize.x, _expandedContentSize.y}, 
                             1.0f, DARKGRAY);
