...
NavigationMenuController::NavigationMenuController(GameStateModel *gameStateModel,
																									 std::shared_ptr<MenuComponent> menuSystem,
...
void NavigationMenuController::updateNavigationMenuForCurrentState() {
	...
	if (shouldUpdate) {
		std::shared_ptr<MenuComponent> newMenuModel =
				_gameStateModel->getCurrentState()->createNavigationMenu(_gameStateModel, _sceneManager);
		if (newMenuModel) {
...
IMenuView *NavigationMenuController::getMenuView() const { return _menuView.get(); }
