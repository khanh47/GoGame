...
SavedGameListMenuController::SavedGameListMenuController(DataManager *dataManager) : _dataManager(dataManager) {
	_menuView = nullptr;
}
...
void SavedGameListMenuController::createSavedGameListMenu() {
	...
	for (const auto &data : gameData) {
		...
		menuItem->setCommand(createSavedGameSelectCommand(_dataManager, data));
