...
ListMenuView::ListMenuView(Rectangle area) : listArea(area) { updateScrollbarArea(); }

void ListMenuView::updateScrollbarArea() {
	scrollbarArea = {listArea.x + listArea.width - scrollbarWidth, listArea.y, scrollbarWidth, listArea.height};
}
...
void ListMenuView::createNavigationItemViews(std::shared_ptr<MenuComponent> menuModel, GameState *gameState) {
	...
	if (gameState == nullptr) {
		for (size_t i = 0; i < menuModel->getChildrens().size(); ++i) {
			...
			if (child) {
				Vector2 position = {listArea.x + 10.0f, listArea.y + i * (itemHeight + itemSpacing)};
...
bool ListMenuView::isScrollbarHovered() const {
	Vector2 mousePos = GetMousePosition();
	Rectangle handleRect = {scrollbarArea.x, scrollbarArea.y + getScrollHandlePosition(), scrollbarArea.width,
...
void ListMenuView::render(std::shared_ptr<MenuComponent> menuModel) const {
	...
	for (size_t i = 0; i < _itemViews.size() && i < menuItems.size(); ++i) {
		if (_itemViews[i] && menuItems[i]->isEnabled()) {
			...
			if (scrolledPos.y + itemHeight >= listArea.y && scrolledPos.y <= listArea.y + listArea.height) {
